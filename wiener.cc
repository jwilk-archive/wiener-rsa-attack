...
// 2. Liczba e wzglêdnie pierwsza z L = NWW(p - 1, q - 1). 
//    (Mo¿na te¿ przyj±æ L = (p - 1)(q -1))
// Wynik:
// 1. Liczba d == e^-1 (mod L)
// 2. Liczby p, q.
//
// Przy za³o¿eniu, ¿e L = (p-1)(q-1), q < p < 2q, atak udaje siê je¿eli 
// d < (n^0.25)/3, e < n, ed > n.
//
// G³ówna idea:
//   Niech
...
//   gdzie
//     t = 1/p + 1/q + 1/pq.
//   Poniewa¿ t jest ma³e, dobrym przybli¿eniem k / dg jest e / n.
...
int main(int argc, char **argv)
{
  ...
  // m: dobre przybli¿enie (p-1)(q-1) = pq - p - q + 1 < m:
  // . pq, lub
  // . floor(pq - 2sqrt(pq) + 1)
  assert(m >= 4);
  ...
  for (unsigned int i = 0; ; i++)
  {
    ...
    if (k == 0)
      break;
    // edg = k(p-1)(q-1) + g, wiêc:
    //   (p-1)(q-1) = edg div k,
    //   g = edg mod k,
    // o ile tylko k > g (na co wystarczy, ¿eby ed > n):
